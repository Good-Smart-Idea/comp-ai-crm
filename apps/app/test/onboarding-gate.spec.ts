...
function setup({
	...
	stub(async (url) => {
		if (!url.includes("workspace.get")) {
			throw new Error(`Unexpected gate request: ${url}`);
		}
...
describe("proxy", () => {
	...
	it("takes a settled rep from onboarding into the workspace", async () => {
		setup();
...
describe("optional Context enrichment", () => {
	it("lets an onboarded rep enter the CRM without a Context key", async () => {
		const calls = setup();
	...
	it("keeps the Context setup page available without making it a gate", async () => {
		setup();
