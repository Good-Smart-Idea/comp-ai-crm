...
export interface CompanyResearchProvider {
	...
	read(url: string): Promise<{ outcome: "found"; text: string; raw: unknown } | { outcome: "failed"; reason: string }>;
}
...
export class BrightDataCompanyResearch implements CompanyResearchProvider {
	...
	async lookup(domain: string): Promise<LookupResult> {
		...
		if (!official) return { outcome: "skipped", reason: "No public company page matched this domain." };
	...
	async read(url: string): Promise<{ outcome: "found"; text: string; raw: unknown } | { outcome: "failed"; reason: string }> {
		let target: URL;
		try {
			target = new URL(url);
		} catch {
		...
		const preflight = await safeFetch(target.toString(), { method: "HEAD", timeoutMs: COMPANY_RESEARCH.request.timeoutMs });
		if (!preflight) return { outcome: "failed", reason: "The company page URL is not public." };
		await preflight.response.body?.cancel();

		const endpoint = this.unlockerUrl();
		const token = this.unlockerToken();
		if (!endpoint || !token) return { outcome: "failed", reason: "Bright Data is not configured." };

		for (let attempt = 0; attempt <= COMPANY_RESEARCH.request.retries; attempt += 1) {
			try {
				const response = await this.fetcher(endpoint, {
					method: "POST",
					headers: { authorization: `Bearer ${token}`, "content-type": "application/json", accept: "text/html,application/json" },
					body: JSON.stringify({ url: preflight.url.toString(), zone: "web_unlocker", format: "raw" }),
					signal: AbortSignal.timeout(COMPANY_RESEARCH.request.timeoutMs),
				});
				if (!response.ok) {
					if (response.status >= 400 && response.status < 500 && response.status !== 429) return { outcome: "failed", reason: `Bright Data answered ${response.status}.` };
					continue;
				}
				const text = (await limitedText(response, COMPANY_RESEARCH.request.maxBytes)).trim();
				return { outcome: "found", text, raw: { provider: "bright-data", url: preflight.url.toString(), content: text } };
			} catch {
				continue;
			}
		}
	...
	private async officialUrl(domain: string): Promise<string | null> {
		...
		const result = await safeFetch(direct, { method: "HEAD", timeoutMs: COMPANY_RESEARCH.request.timeoutMs });
		if (result) {
		...
		try {
			const response = await this.fetcher(endpoint, {
				...
				body: JSON.stringify({ query: domain, num_results: COMPANY_RESEARCH.request.maxPages }),
				signal: AbortSignal.timeout(COMPANY_RESEARCH.request.timeoutMs),
			});
			if (!response.ok) return null;
			const body = (await response.json()) as { organic?: { link?: unknown }[]; results?: { url?: unknown }[] };
			const candidates = [...(body.organic ?? []).map((row) => row.link), ...(body.results ?? []).map((row) => row.url)];
			for (const candidate of candidates.slice(0, COMPANY_RESEARCH.request.maxPages)) {
				if (typeof candidate !== "string") continue;
				const page = await safeFetch(candidate, { method: "HEAD", timeoutMs: COMPANY_RESEARCH.request.timeoutMs });
				if (!page) continue;
				const matches = page.url.hostname === domain || page.url.hostname.endsWith(`.${domain}`);
				await page.response.body?.cancel();
				if (matches) return page.url.toString();
			}
		} catch {}
		return null;
	}
	...
	private unlockerUrl(): string | null {
		return process.env.BRIGHT_DATA_WEB_UNLOCKER_URL?.trim() || process.env.BRIGHT_DATA_ISP_URL?.trim() || null;
	}

	private unlockerToken(): string | null {
		return process.env.BRIGHT_DATA_WEB_UNLOCKER_API_KEY?.trim() || process.env.BRIGHT_DATA_ISP_API_KEY?.trim() || null;
	}
}
...
export function brandFromPage(domain: string, url: string, page: string): Brand {
	const title = content("og:title", page) ?? /<title[^>]*>([^<]+)<\/title>/i.exec(page)?.[1]?.trim() ?? null;
	const description = content("description", page) ?? content("og:description", page);
	const logo = content("og:image", page);
	const email = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i.exec(page)?.[0] ?? null;
	...
	return {
		...
		email,
		phone,
		logos: logo ? [{ url: new URL(logo, url).toString(), type: "logo", mode: "light" }] : [],
		socials,
